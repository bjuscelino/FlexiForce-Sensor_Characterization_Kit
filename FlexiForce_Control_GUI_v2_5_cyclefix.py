...
class App(tk.Tk):
    def __init__(self):
        ...
        # Controle do modo de repetibilidade (ciclos)
    ...
    def start_test(self, module_name: str):
        """Inicia um teste para o módulo selecionado.

        Para o módulo "repeat" (repetibilidade) gerenciamos os ciclos
        no próprio PC, repetindo o teste de LIN_HYST N vezes. Isso evita
        depender de detalhes da firmware e garante que o modo de ciclo
        funcione mesmo em versões antigas do Arduino.
        """
        ...
        for tab_id, mod in self._tab_module.items():
            if mod == module_name:
                ...
                break
        # Aponta self.data para o módulo escolhido
        if module_name in self.data_by_module:
            self.data = self.data_by_module[module_name]
        # Limpa dados e gráficos da aba atual
        self.clear_graph(module_name)

        if module_name == "repeat":
            # Modo especial: repetibilidade em ciclos
            self._start_repeat_test()
        else:
            # Demais módulos usam a sequência direta
    ...
    def _run_test_sequence(self, module_name: str):
        """Envia o comando de teste correspondente ao módulo simples.

        Usado para linearidade, histerese e drift. O modo "repeat" usa
        um gerenciador próprio (_start_repeat_test / _start_next_repeat_cycle).
        """
    ...
    def _start_repeat_test(self):
        """Configura o modo de repetibilidade e dispara o primeiro ciclo.

        Implementamos o ciclo no PC repetindo o teste de LIN_HYST
        N vezes. Assim o modo de ciclo funciona mesmo em firmwares
        que não suportam bem TEST:REPEAT / SET:REPEAT.
        """
        # Garante que haja uma conexão válida
        if not self.serial.is_connected():
        ...
        try:
            # spinbox normalmente já retorna int, mas garantimos
            n = int(n)
        except Exception:
            n = 1
        if n < 1:
    ...
    def _start_next_repeat_cycle(self):
        """Inicia o próximo ciclo LIN_HYST dentro do modo repeat."""
        ...
        # Usa diretamente o comando de histerese
            ...
            while True:
                ...
                if self.acq:
                    ...
                    if self._parse_and_add(line):
                        ...
                        # Linha não numérica (OK, DONE, etc.) ou formato inesperado
    ...
    def _handle_control_line(self, line: str):
        """Trata mensagens de controle vindas da serial (OK, DONE, etc.).

        Também é aqui que avançamos os ciclos do modo de repetibilidade
        quando o firmware sinaliza o fim de um teste LIN_HYST.
        """
        if line is None:
            return
        txt = line.strip()
        ...
        if "DONE LIN_HYST" in up:
            # Um ciclo de histerese terminou
            self.log("Fim de um ciclo LIN_HYST.")
            if self.repeat_mode_active and self.repeat_cycle_running:
                # Dispara o próximo ciclo do teste de repetibilidade
                self.repeat_cycle_running = False
                self._start_next_repeat_cycle()
            return

        if "DONE REPEAT" in up:
            # Caso esteja usando diretamente TEST:REPEAT em alguma versão
            self.log("Fim do teste REPEAT reportado pelo firmware.")
            self.repeat_mode_active = False
            self.repeat_cycle_running = False
            return

        # Mensagens OK ou outras respostas de texto – logamos apenas no início
        if self.data.valid_lines < 5:
            self.log(f"<<< {txt}")
