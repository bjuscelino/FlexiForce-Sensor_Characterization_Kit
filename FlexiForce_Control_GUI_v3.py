...
APP_TITLE   = "FlexiForce Control GUI v3"
BAUDRATE    = 9600
READ_TIMEOUT= 0.1
UPDATE_MS   = 100
...
class App(tk.Tk):
    ...
    def _build_ui(self):
        ...
        self.main_notebook.grid(row=0, column=0, sticky="nsew", padx=6, pady=6)

        tab_cfg = ttk.Frame(self.main_notebook)
        tab_tests = ttk.Frame(self.main_notebook)
        self.main_notebook.add(tab_cfg, text="Configuração")
        self.main_notebook.add(tab_tests, text="Testes")

        tab_cfg.columnconfigure(0, weight=1)

        top = ttk.Frame(tab_cfg, padding=6)
        ...
        instr_frame = ttk.LabelFrame(tab_cfg, text="Instruções de uso", padding=8)
        instr_frame.grid(row=1, column=0, sticky="nsew", padx=6, pady=(4,6))
        tab_cfg.rowconfigure(1, weight=1)

        instr_text = (
            "🔌 Conexão de hardware:\n"
            "  • Conecte a placa (Arduino / controle FlexiForce) ao PC via USB.\n"
            "  • Conecte o sensor FlexiForce ao divisor resistivo conforme o esquema do projeto.\n"
            "\n"
            #"🔍 Multímetro em modo continuidade (obrigatório para medir a resistência):\n"
            #"  • Coloque o multímetro em modo CONTINUIDADE (ou teste de diodo).\n"
            #"  • Deixe as pontas do multímetro conectadas no ponto indicado do divisor de tensão.\n"
            #"  • Sem o multímetro em continuidade o sistema NÃO consegue ler corretamente a\n"
            #"    resistência do sensor durante as aquisições de dados.\n"
            #"\n"
            "📟 Configuração inicial:\n"
            "  1. Escolha a porta serial correta e clique em «Conectar».\n"
            "  2. Selecione o preset do sensor ou ajuste manualmente a área (mm²).\n"
            "  3. Ajuste N/unid. da célula de carga, R_top, R_bot e Vref se necessário.\n"
            "  4. Se tiver um valor conhecido de resistência, informe em «R_medido» e use «Calibrar!». \n"
            "\n"
            "📊 Execução dos testes:\n"
            "  • Vá para a aba «Testes».\n"
            "  • Escolha o módulo (Linearity, Lin+Hyst, Drift ou Repeat).\n"
            "  • Clique em «Iniciar teste» no módulo desejado.\n"
            "  • Use «Iniciar Aquisição» / «Parar Aquisição» para controlar a coleta de dados.\n"
            "  • Use «Exportar CSV» para salvar os resultados em arquivo.\n"
        )
        lbl_instr = ttk.Label(instr_frame, text=instr_text, justify="left", anchor="nw", wraplength=1200)
        lbl_instr.pack(fill="both", expand=True)

        tab_tests.columnconfigure(0, weight=1)
        tab_tests.rowconfigure(2, weight=1)
        tab_tests.rowconfigure(4, weight=1)

        ctrl=ttk.LabelFrame(tab_tests,text="Controle e Status",padding=6)
        ...
        acq_frame = ttk.Frame(tab_tests, padding=(6,4))
        acq_frame.grid(row=1, column=0, sticky="ew")
        acq_frame.columnconfigure(3, weight=1)

        self.btn_scale=ttk.Button(acq_frame,text="Escala: Linear",command=self.on_toggle_scale)
        self.btn_scale.grid(row=0,column=0,padx=4,pady=2)
        ttk.Button(acq_frame,text="Exportar CSV",command=self.on_export_csv).grid(row=0,column=1,padx=4,pady=2)
        self.btn_acq=ttk.Button(acq_frame,text="Iniciar Aquisição",command=self.on_toggle_acq,state="disabled")
        self.btn_acq.grid(row=0,column=2,padx=4,pady=2)

        ttk.Label(acq_frame,text="R_medido (Ω):").grid(row=0,column=3,sticky="e",padx=(15,2))
        self.ed_rmeas=ttk.Entry(acq_frame,width=10)
        self.ed_rmeas.grid(row=0,column=4,padx=2)

        ttk.Button(acq_frame,text="Calibrar!",command=self.on_calibrate)            .grid(row=0,column=5,padx=4)

        self.notebook = ttk.Notebook(tab_tests)
        self.notebook.grid(row=2, column=0, sticky="nsew", padx=0, pady=(4,0))
        ...
        logbar = ttk.Frame(tab_tests)
        logbar.grid(row=3, column=0, sticky="ew", padx=0, pady=(4,0))
        self.btn_logtoggle = ttk.Button(logbar, text="Esconder LOG", command=self.on_toggle_log)
        self.btn_logtoggle.pack(side="right")

        log = ttk.LabelFrame(tab_tests, text="Log", padding=6)
        log.grid(row=4, column=0, sticky="nsew", padx=0, pady=(0,6))
        ...
        self.status = ttk.Label(self, text="By: Juscelino Valter Barbosas Junior")
        self.status.grid(row=1, column=0, sticky="e", padx=6, pady=(0,6))
    ...
    def _on_tab_changed(self, event):
        ...
        if module:
            ...
            # Recalibração automática ao trocar módulo
            try:
                if self.ed_rmeas.get().strip() != "":
                    self.on_calibrate()
            except Exception:
                pass
    ...
    def _start_repeat_test(self):
        if not self.serial.is_connected():
            self.log("Não é possível iniciar o teste de repetibilidade: não conectado.")
            return
        ...
        self._start_acq(reason=f"(repetibilidade, {n} ciclos)")
        self.log(f"Iniciando teste de repetibilidade: {n} ciclo(s) de LIN_HYST.")
    ...
    def _start_next_repeat_cycle(self):
        if not self.repeat_mode_active:
            return
        if not self.serial.is_connected():
            self.log("Conexão perdida durante o teste de repetibilidade.")
            self.repeat_mode_active = False
            self.repeat_cycle_running = False
            return

        if self.repeat_cycle_index >= self.repeat_cycle_total:
            self.log("✅ Teste de repetibilidade concluído.")
            self.repeat_mode_active = False
            self.repeat_cycle_running = False
            return
    ...
    def _handle_control_line(self, line: str):
        ...
        if "DONE LIN_HYST" in up:
            self.log("Fim de um ciclo LIN_HYST.")
        ...
        if "DONE REPEAT" in up:
            self.log("Fim do teste REPEAT reportado pelo firmware.")
            self.repeat_mode_active = False
            self.repeat_cycle_running = False
            return
